...
export function useSuppliers(options:{api:any,data:any,filters:any,editing:any,modal:Ref<null|'item'|'customer'|'supplier'>,submit:Submit,softDelete:SoftDelete}){
  const {api,data,filters,editing,modal,submit,softDelete}=options
  ...
  async function removeSupplier(v:any){const result=await Swal.fire({icon:'warning',title:`Hapus ${v.name}?`,showCancelButton:true,confirmButtonText:'Hapus',cancelButtonText:'Batal',confirmButtonColor:'#b8322a'});if(result.isConfirmed)await softDelete(()=>api.deleteSupplier(v.id),()=>api.restoreSupplier(v.id),`Supplier ${v.name} dihapus`,[loadSuppliers])}
