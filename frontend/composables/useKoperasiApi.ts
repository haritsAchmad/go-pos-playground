...
export function useKoperasiApi() {
  ...
  return {
    ...
    deletedItems: () => request<any[]>('/deleted/items'),
    suppliers: () => request<any[]>('/suppliers'),
    deletedSuppliers: () => request<any[]>('/deleted/suppliers'),
    customers: () => request<any[]>('/customers'),
    deletedCustomers: () => request<any[]>('/deleted/customers'),
    ...
    restoreSupplier: (id: number) => request(`/suppliers/${id}/restore`, { method: 'POST' }),
    ...
    bulkSettleDebts: (ids: number[]) => request<any>('/bulk/debts/settle', { method: 'POST', body: { ids } }),
    bulkResetStock: (ids: number[]) => request<any>('/bulk/items/reset-stock', { method: 'POST', body: { ids } }),
