...
const { downloadCsv, parseCsv } = useCsvData()
const nav = [
...
function exportData(kind:'items'|'customers'|'suppliers'|'transactions'){
  const date=new Date().toISOString().slice(0,10)
  if(kind==='items')downloadCsv(`barang-${date}.csv`,['sku','name','description','supplier_code','category','brand','unit','stock','cost','price'],data.items.map((v:any)=>[v.sku,v.name,v.description,data.suppliers.find((s:any)=>s.id===v.supplier_id)?.code||'',v.category_name||'',v.brand_name||'',v.unit_name||'',v.stock,v.cost,v.price]))
  if(kind==='customers')downloadCsv(`pelanggan-${date}.csv`,['code','name','phone','address'],data.customers.map((v:any)=>[v.code,v.name,v.phone,v.address]))
  if(kind==='suppliers')downloadCsv(`supplier-${date}.csv`,['code','name','phone','address'],data.suppliers.map((v:any)=>[v.code,v.name,v.phone,v.address]))
  if(kind==='transactions')downloadCsv(`histori-transaksi-${date}.csv`,['invoice','date','type','party','status','payment_status','total','received','change','items','notes'],filteredTransactions.value.map((v:any)=>[v.invoice_no,v.transaction_date,v.transaction_type,v.customer_name||v.supplier_name||'',v.status,v.payment_status,v.grand_total,v.amount_received,v.change_amount,v.items.map((x:any)=>`${x.item_name} (${x.quantity} x ${x.unit_price})`).join('; '),v.notes]))
}
...
async function importData(kind:'items'|'customers'|'suppliers',event:Event){
  ...
  try{
    const rows=parseCsv(await file.text());if(!rows.length)throw new Error('CSV kosong atau formatnya tidak sesuai template export.')
    ...
    for(let index=0;index<rows.length;index++){
      ...
      try{
        if(kind==='customers'){
          if(!row.code||!row.name)throw new Error('code dan name wajib diisi');if(String(row.code).toUpperCase()==='UMUM')continue
          const existing=data.customers.find((v:any)=>v.code.toLowerCase()===row.code.toLowerCase());const body={code:row.code,name:row.name,phone:row.phone||'',address:row.address||''};await (existing?api.updateCustomer(existing.id,body):api.createCustomer(body))
        }else if(kind==='suppliers'){
          if(!row.code||!row.name)throw new Error('code dan name wajib diisi')
          const existing=data.suppliers.find((v:any)=>v.code.toLowerCase()===row.code.toLowerCase());const body={code:row.code,name:row.name,phone:row.phone||'',address:row.address||''};await (existing?api.updateSupplier(existing.id,body):api.createSupplier(body))
        }else{
          ...
          const body={sku:row.sku,name:row.name,description:row.description||'',supplier_id:supplier?.id||null,category_id:findByName(data.categories,row.category),brand_id:findByName(data.brands,row.brand),unit_id:findByName(data.units,row.unit),stock:Number(row.stock||0),cost:Number(row.cost||0),price:Number(row.price||0)}
          const existing=data.items.find((v:any)=>v.sku.toLowerCase()===row.sku.toLowerCase());await (existing?api.updateItem(existing.id,body):api.createItem(body))
        }
      }catch(e:any){throw new Error(`Baris ${index+2}: ${e?.data?.message||e.message}`)}
    }
    await load();await Swal.fire({icon:'success',title:'Import selesai',text:`${rows.length} baris berhasil diproses.`,confirmButtonColor:'#1d6b43'})
  ...
  <div class="app-shell">
    ...
    <main @keydown="blockInvalidNumber" @input="sanitizeNumeric">
      ...
      <template v-else-if="active==='items'">
        <div class="page-heading"><div><h2>Barang</h2><p>Kelola katalog dan persediaan barang.</p></div><div class="heading-actions"><input ref="itemImport" class="file-input" type="file" accept=".csv,text/csv" @change="importData('items',$event)"><button class="soft" @click="itemImport?.click()">Import CSV</button><button class="soft" @click="exportData('items')">Export CSV</button><button class="primary" @click="openItem()">+ Tambah Barang</button></div></div>
      ...
      <template v-else-if="active==='customers' || active==='suppliers'">
        <div class="page-heading"><div><h2>{{active==='customers'?'Pelanggan':'Supplier'}}</h2><p>Kelola data {{active==='customers'?'pelanggan dan anggota':'mitra pemasok'}}.</p></div><div class="heading-actions"><input v-if="active==='customers'" ref="customerImport" class="file-input" type="file" accept=".csv,text/csv" @change="importData('customers',$event)"><input v-else ref="supplierImport" class="file-input" type="file" accept=".csv,text/csv" @change="importData('suppliers',$event)"><button class="soft" @click="active==='customers'?customerImport?.click():supplierImport?.click()">Import CSV</button><button class="soft" @click="exportData(active==='customers'?'customers':'suppliers')">Export CSV</button><button class="primary" @click="openParty(active==='customers'?'customer':'supplier')">+ Tambah {{active==='customers'?'Pelanggan':'Supplier'}}</button></div></div>
