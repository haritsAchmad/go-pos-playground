...
func NewManager(secret, issuer, expiryMinutes string) (*Manager, error) {
	...
	if issuer == "" {
		issuer = "go-pos-playground"
	}
	minutes := 480
	if expiryMinutes != "" {
		v, err := strconv.Atoi(expiryMinutes)
		if err != nil || v <= 0 {
			return nil, errors.New("JWT_EXPIRY_MINUTES must be a positive integer")
		}
		minutes = v
	}
...
func (m *Manager) Issue(id, sessionID int64, name, email, role string) (string, int64, error) {
	if sessionID < 1 {
	...
	payload, err := json.Marshal(Claims{fmt.Sprint(id), sessionID, name, email, role, m.issuer, now.Unix(), exp})
	if err != nil {
		return "", 0, err
	}
...
func (m *Manager) Parse(token string) (Claims, error) {
	...
	if json.Unmarshal(payload, &claims) != nil || claims.Issuer != m.issuer || claims.Subject == "" || claims.SessionID < 1 || time.Now().Unix() >= claims.ExpiresAt {
