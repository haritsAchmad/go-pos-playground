...
func Authenticate(tokens *auth.Manager, users *repository.AuthRepository, next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		activeSession, err := users.IsSessionActive(r.Context(), claims.SessionID, userID)
		if err != nil || !activeSession {
			response.Error(w, http.StatusUnauthorized, "session is expired, revoked, or invalid")
			return
		}
		user, err := users.FindUserByID(r.Context(), userID)
		if err != nil || !user.Active {
			response.Error(w, http.StatusUnauthorized, "account is unavailable or inactive")
			return
		}
